...
set.seed(42)
in_dir_ = '../data/integration/cancer/'
out_dir_ = '../out/integration/cancer/treated_untreated/condition/'
fl_nm = 'Milo_DA_k30_d30_prop0.4_cancer'
...
load(file = list.files(path = in_dir_, pattern = 'integrated_compartment_cancer_processed.RData', full.names = T))
DefaultAssay(s_objs) = 'integrated'
s_objs$cell_type = s_objs$cell_type_2
...
for(ds_ in c('condition'))
{
  ...
  da_results$cell_type = ifelse(da_results$cell_type_fraction < .6, "Mixed", da_results$cell_type)      # if the fraction of the most abundant cell type in a given neighborhood is less than cutoff,
                                                                                                        # that neighborhood is not homogeneous
  da_results$cell_type = factor(x = da_results$cell_type, levels = rev(c('mitotic blastema 2',
                                                                         'blastema 2',
                                                                         'mitotic blastema 1',
                                                                         'blastema 1',
                                                                         'blastema 3',
